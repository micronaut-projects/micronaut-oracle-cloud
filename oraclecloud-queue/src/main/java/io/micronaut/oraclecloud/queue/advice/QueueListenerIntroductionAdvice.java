...
/**
 * Introduction advice for {@link QueueListener}.
 */
@Singleton
@InterceptorBean(QueueListener.class)
public class QueueListenerIntroductionAdvice extends AbstractQueueIntroductionAdvice<QueueListener> {

    QueueListenerIntroductionAdvice(QueueService service,
                                    OracleQueueConfiguration config) {
        super(service, config, QueueListener.class);
    }

    @Override
    protected boolean isSupportedDeclaringType(Class<?> type) {
        return type.equals(GenericQueue.class) || type.equals(GenericQueueListener.class);
    }
}
