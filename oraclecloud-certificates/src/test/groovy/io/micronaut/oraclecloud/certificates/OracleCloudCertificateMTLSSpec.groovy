...
class OracleCloudCertificateMTLSSpec extends Specification {
    ...
    def  "mtls handshake should succeed with default oci config provider"() {
        given:
        def props = [
                "micronaut.server.ssl.enabled": true,
                "micronaut.server.ssl.port": "0",
        ...
        def server = ctx.getBean(EmbeddedServer)
        server.start()
        ObjectStorage client = server.getApplicationContext().getBean(ObjectStorage.class)

        when:
        def response = client.getNamespace(GetNamespaceRequest.builder().build())

        then:
        response.getValue().contains("CN=micronaut.guide.x509")
    }

    def  "mtls handshake should succeed with specific objectstorage oci config provider"() {
        given:
        def props = [
                "micronaut.server.ssl.enabled": true,
                "micronaut.server.ssl.port": "0",
                ...
                "micronaut.server.ssl.key-name": "server",
                "micronaut.server.ssl.trust-name": "server",
                "oci.clients.objectstorage.ssl.trust-name": "client",
                "oci.clients.objectstorage.ssl.key-name": "client",
                "oci.clients.objectstorage.ssl.enabled": "true",
                "oci.clients.certificates.ssl.insecure-trust-all-certificates": "true",
                "test.client.url": embeddedServer.URI.toString()
        ]
        def ctx = ApplicationContext.builder().properties(props).start()
    ...
    @Controller("/n")
    static class ObjectStorageController {

        @Get("/")
        @Secured(IS_ANONYMOUS)
        String getCertificateBundleResponse(@Nullable X509Authentication x509Auth,
                                                       @Nullable Authentication authentication) {
            if (!x509Auth && !authentication) {
                return "Hello unknown!";
            }
