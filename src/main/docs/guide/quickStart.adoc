...
Then add dependencies on the Micronaut Oracle Cloud modules corresponding to OCI SDK clients you wish to use, for example, Vault:

dependency:io.micronaut.oraclecloud:micronaut-oraclecloud-bmc-vault[]

Use Micronaut Injection to get an instance of the client in your application, e.g.:

snippet::example.VaultService[project-base="docs-examples/example", source="main", tags="inject", indent="0"]
