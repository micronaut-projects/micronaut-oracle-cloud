...
[.lang-python]
WARNING: The Fn runtime instantiates the function class itself, before any Micronaut application context, and with it the Python runtime, exists, and a Python class cannot subclass api:oci.function.OciFunction[], which starts the application context. A Python function class like the one above is therefore only usable as a bean of a running application (for example in a test); the class deployed as the Oracle Function entry point must be written in Java, Kotlin or Groovy.
