...
Provides common functionality and configures the following beans:

* link:{ocidocs}/com/oracle/bmc/auth/BasicAuthenticationDetailsProvider.html[BasicAuthenticationDetailsProvider] - Authentication details auto-detected from local `$HOME/.oci/config` or environment
* link:{ocidocs}/com/oracle/bmc/ClientConfiguration.html[ClientConfiguration] - Common configuration for all clients
* api:oci.core.TenancyIdProvider[] - Interface to resolve the current tenancy
...
NOTE: The module is only required when you want to use the OCI SDK clients with a Jersey implementation, as the Netty client implementations support Micronaut injection by default.
For example, to use Vault with the OCI Jersey client, you would define a dependency on this module as well as:
...
Provides support for injecting RxJava 2 client implementations.
...
Provides support for injecting Reactor client implementations.
...
Provides the api:oci.function.OciFunction[] superclass that you can subclass to write Oracle Cloud Functions in Micronaut that can use Micronaut dependency injection.

=== micronaut-oraclecloud-function-http

dependency:io.micronaut.oraclecloud:micronaut-oraclecloud-function-http[]

Provides the api:oci.function.http.HttpFunction[] class that can be used as a handler to write HTTP API Gateway function handlers that delegate to Micronaut controllers.
...
=== micronaut-oraclecloud-notifications

dependency:io.micronaut.oraclecloud:micronaut-oraclecloud-notifications[]

Provides support for publishing messages to https://docs.oracle.com/en-us/iaas/Content/Notification/Concepts/notificationoverview.htm[Oracle Cloud Notifications] topics.
