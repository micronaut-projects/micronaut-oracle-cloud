...
All other properties, in particular those related to proxies or TLS configuration, are not supported.
...
The service ID can be customized using the `NettyClientProperties.SERVICE_ID` property to `my_new_id` for an example or by setting `oci.clients.identity.serviceId=my_new_id`. You can then configure this client further using `micronaut.http.services.my_new_id.*` properties.
